...
// =========================================================
// 1. Intermediate Code (Quadruples) Structure
// =========================================================
typedef struct {
    char op[16];     // Operation code (e.g., ADD, CALL, JMP_F, JMP)
    char arg1[32];   // First operand
    char arg2[32];   // Second operand
    char result[32]; // Result or Jump Target
} Quad;

Quad quads[1000];    // Array to store all instructions
int quad_count = 0;  // Instruction counter
...
// =========================================================
typedef enum {
    TK_FUNC, TK_RETURN, TK_IF, TK_WHILE, // <--- Added TK_WHILE
    TK_ID, TK_NUM, 
...
void next_token() {
    while (1) {
        ...
        if (*src == '/') {
            if (*(src + 1) == '/') { 
                ...
                src += 2; 
                while (!(*src == '*' && *(src + 1) == '/') && *src != '\0') src++;
                if (*src != '\0') src += 2; continue; 
            }
        }
        break;
    }
    ...
    // Identifiers and Keywords
        ...
        else if (strcmp(cur_token.text, "while") == 0) cur_token.type = TK_WHILE; // <--- Added 'while' check
    ...
    // Symbols
        ...
        switch (*src++) {
            ...
            case '=':
                ...
                else cur_token.type = TK_ASSIGN; break;
        }
    }
}
...
void expression(char *res) {
    char l[32], r[32], t[32]; arith_expr(l);
...
void statement() {
    ...
    // WHILE Statement (Added Implementation)
    } else if (cur_token.type == TK_WHILE) {
        next_token(); // skip 'while'
        next_token(); // skip '('
        
        int loop_start = quad_count; // Record the start label
        char cond[32]; 
        expression(cond); // Parse the condition
        
        next_token(); // skip ')'
        next_token(); // skip '{'
        
        int exit_jump_idx = quad_count;
        emit("JMP_F", cond, "-", "?"); // If false, jump out
        ...
        char start_str[32];
        sprintf(start_str, "%d", loop_start);
        emit("JMP", "-", "-", start_str); // Unconditional jump back to start
        
        next_token(); // skip '}'
        sprintf(quads[exit_jump_idx].result, "%d", quad_count); // Fix the exit jump address
...
void vm() {
    int pc = 0; 
    int param_stack[1000]; int param_sp = 0; 
    int func_pc[100]; char func_names[100][32]; int f_count = 0;
    
    // Pre-scan function addresses
    ...
    while (pc < quad_count) {
        ...
        else if (strcmp(q.op, "JMP") == 0) { // <--- Added JMP logic
            pc = atoi(q.result) - 1;
        }
        else if (strcmp(q.op, "PARAM") == 0) {
            param_stack[param_sp++] = get_var(q.arg1); 
        }
...
int main(int argc, char *argv[]) {
    ...
    src = buffer;
    printf("Compiler Generated Quadruples (Intermediate Code):\n");
    printf("--------------------------------------------------\n");
